...
The second is a subset with additional observed kinematic constaints 
...
these alignments and recent claims of an universal subhalo accretion
direction. 
This highlights the relevance of achieving a precise characterization
of the place of the LG in the cosmic web in a the cosmological context
...
agreement with a low pairs frequency aswell.  
...
sheets, $1/4$ in peaks, $1/4$ in filaments and the remaning $1/6$ in
voids.  
...
amples is very narrow and it is used to constraint the LG mass.
...
samples.   
In thre three cases it is evident that the range of values for the
...
the $\hat{n}$ vector, the increasing strenght of the
...
This is consistent with our results of no strong alignement.
...
have shown that the local overdensity is the dominant web parameter
that define the abundance and properties of halos. Other quantities
derived from the tidal tensor play a secondary role defining
properties such as its formation history.
...
value. 
Correspondignly, the values for the ellipticity and prolateness are
...
However, the alignments with the cosmic web that are not
mass dependent and seem to be stronger once the kinematic constraints
...
However, such alignments can be traced in observations by looking for 
similar pairs samples (or even a more unconstrained pair sample) in galaxy surveys and study the angle between
pair axis and surrounding filament computed using a filament finder, 
such as codes presented by \citet{2010MNRAS.407.1449G,2011MNRAS.414..350S}.

These alignments for the LG pairs could be in contradiction with
recent findings \citep{2014MNRAS.443.1274L} that report a universal
trend where the eigenvector $\hat{e}_{3}$ marks the infall direction
for substructures. For the Milky Way its satellites are located at
high galactic latitudes, almost perpendicular to the vector pointing towards
the Andromeda Galaxy, which is the direction that we find aligned with
$\hat{e}_3$. 

If the alignment we find in this paper holds, this would imply that
infall in the case of the Milky Way is perpendicular to $\hat{e}_3$,
in contradiction to the universal principle of
\citep{2014MNRAS.443.1274L}. If the universal accretion alignment
holds, that would imply that that the Andromeda galaxy is located
perpendicular to $\hat{e}_3$, in contradiction with the strong
alignment found in this paper. It would be desirable to have a
measurement of the cosmic web in the Local Universe to disentangle
this potential contradiction between different alignments and
understading the detailed properties of the LG in an explicit
